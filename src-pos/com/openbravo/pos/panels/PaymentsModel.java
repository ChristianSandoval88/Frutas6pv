...
public class PaymentsModel {
 private String user;
    ...
    private java.util.List<PaymentsLine> m_lpayments2;
    ...
    public static PaymentsModel emptyInstance() {
        ...
        p.m_lpayments2 = new ArrayList<PaymentsLine>();
...
String dateNow = formatter.format(currentDate.getTime());
    ...
    valtickets = (Object[])(new StaticSentence(app.getSession(), "SELECT COUNT(*), SUM(PAYMENTS.TOTAL) FROM PAYMENTS, RECEIPTS WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEY = ? AND PAYMENTS.PAYMENT<>'magcard' AND PAYMENTS.PAYMENT<>'debt' AND PAYMENTS.PAYMENT<>'debtpaid'  AND PAYMENTS.PAYMENT<>'debtpaid2'", SerializerWriteString.INSTANCE, (SerializerRead)new SerializerReadBasic(new Datas[] { Datas.INT, Datas.DOUBLE }))).find(app.getActiveCashIndex());
    if (valtickets == null) {
      p.m_iPayments = new Integer(0);
      p.m_dPaymentsTotal = new Double(0.0D);
    } else {
      p.m_iPayments = (Integer)valtickets[0];
      p.m_dPaymentsTotal = (Double)valtickets[1];
    } 
    ...
    public static PaymentsModel loadInstance(AppView app) throws BasicException {
        ...
        Object[] valtickets = (Object []) new StaticSentence(app.getSession()
            , "SELECT COUNT(*), SUM(PAYMENTS.TOTAL) " +
              "FROM PAYMENTS, RECEIPTS " +
              "WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEY = ?"+
              " AND PAYMENTS.PAYMENT<>'debt' AND PAYMENTS.PAYMENT<>'debtpaid'  AND PAYMENTS.PAYMENT<>'debtpaid2'"
            , SerializerWriteString.INSTANCE
            , new SerializerReadBasic(new Datas[] {Datas.INT, Datas.DOUBLE}))
        ...
        List l2 = new StaticSentence(app.getSession()            
            , "SELECT PAYMENTS.PAYMENT, SUM(PAYMENTS.TOTAL) " +
              "FROM PAYMENTS, RECEIPTS " +
              "WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEY = ? " +
              " AND PAYMENTS.PAYMENT LIKE 'debt%' "+
              "GROUP BY PAYMENTS.PAYMENT"
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentsModel.PaymentsLine.class)) //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
            .list(app.getActiveCashIndex()); 
        ...
        List products = new StaticSentence(app.getSession(), "SELECT PRODUCTS.NAME, SUM(TICKETLINES.UNITS), PRODUCTS.PRICEBUY,SUM(TICKETLINES.UNITS*TICKETLINES.PRICE),SUM((TICKETLINES.UNITS*TICKETLINES.PRICE)-(TICKETLINES.UNITS*PRODUCTS.PRICEBUY))  FROM TICKETLINES, TICKETS, RECEIPTS, PRODUCTS, PAYMENTS WHERE PAYMENTS.RECEIPT=RECEIPTS.ID AND TICKETLINES.PRODUCT = PRODUCTS.ID AND TICKETLINES.TICKET = TICKETS.ID AND TICKETS.ID = RECEIPTS.ID AND RECEIPTS.MONEY = ? GROUP BY PRODUCTS.NAME", SerializerWriteString.INSTANCE, new SerializerReadClass(ProductSalesLine.class)).list(app.getActiveCashIndex());
    if (products == null) {
      p.m_lproductsales = new ArrayList();
    } else {
      p.m_lproductsales = products;
...
String dateNow = formatter.format(currentDate.getTime());
    ...
    if (stock2 == null) {
      ...
      p.m_lStock2 = stock2;
        ...
        valtickets = (Object[])(new StaticSentence(app.getSession(), "SELECT COUNT(*), SUM(PAYMENTS.TOTAL) FROM PAYMENTS, RECEIPTS WHERE PAYMENTS.RECEIPT = RECEIPTS.ID AND RECEIPTS.MONEY = ? AND PAYMENTS.PAYMENT<>'magcard' AND PAYMENTS.PAYMENT<>'cheque' AND PAYMENTS.PAYMENT<>'debt' AND PAYMENTS.PAYMENT<>'debtpaid'  AND PAYMENTS.PAYMENT<>'debtpaid2'", SerializerWriteString.INSTANCE, (SerializerRead)new SerializerReadBasic(new Datas[] { Datas.INT, Datas.DOUBLE }))).find(app.getActiveCashIndex());
    if (valtickets == null) {
      p.m_iPayments = new Integer(0);
      p.m_dPaymentsTotal = new Double(0.0D);
    } else {
...
public static class ProductSalesLine
    implements SerializableRead
  {
    ...
    public List<PaymentsLine> getPaymentLines2() {
        return m_lpayments2;
    }
